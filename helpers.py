...
class DietEnv:
    def __init__(self, df_sarapan, df_camilan, df_makan_siang, df_makan_malam, energy_breakdown, daily_protein,
                 daily_carbs, daily_fats):
    ...
    def _take_action(self, action):
        ...
        if self.current_step in [1, 3, 5]:  # Camilan
            ...
            if valid_snacks:
                # Pilih 5 camilan dengan jarak terdekat
                distances = [(np.sqrt(
                ...
                best_foods = distances[:5]  # Ambil 5 camilan dengan jarak terdekat
                selected_food = random.choice(best_foods)[1]
                self.selected_snacks.add(selected_food['nama_resep'])
            else:
                selected_food = None
        else:
            valid_foods = []
            ...
            if valid_foods:
                ...
                distances.sort(key=lambda x: x[0])
                best_foods = distances[:5]  # Ambil 5 makanan dengan jarak terdekat
                selected_food = random.choice(best_foods)[1]
            else:
                selected_food = None
        ...
        if selected_food is not None:
            ...
            self.current_meal_name = selected_food['nama_resep']
        else:
            self.total_rating -= 1
            self.current_meal_name = None
    ...
    def _get_current_dataframe(self):
        ...
        if self.current_step == 0:
            return self.df_sarapan[
                (self.df_sarapan['kalori'] <= max_energy) & (self.df_sarapan['lemak'] <= max_fats_allowed)]
        elif self.current_step in [1, 3, 5]:
            return self.df_camilan[
                (self.df_camilan['kalori'] <= max_energy) & (self.df_camilan['lemak'] <= max_fats_allowed) & (
                    ~self.df_camilan.index.isin(self.selected_snacks))]
        elif self.current_step == 2:
            return self.df_makan_siang[
                (self.df_makan_siang['kalori'] <= max_energy) & (self.df_makan_siang['lemak'] <= max_fats_allowed)]
        elif self.current_step == 4:
            return self.df_makan_malam[
                (self.df_makan_malam['kalori'] <= max_energy) & (self.df_makan_malam['lemak'] <= max_fats_allowed)]
    ...
    def _calculate_reward(self):
        ...
        normalized_reward = (
                                        energy_reward + protein_reward + carbs_reward + fats_reward + rating_reward + distance_reward) / 6

        # Penalti jika makanan yang sama dipilih berulang kali
        if self.current_meal_name in self.selected_snacks:
    ...
    def action_space_size(self):
        ...
        df_sarapan_filtered = self.df_sarapan[(self.df_sarapan['kalori'] <= self.energy_breakdown['sarapan']) & (
                    self.df_sarapan['lemak'] <= max_fats_allowed)]
        df_camilan_filtered = self.df_camilan[(self.df_camilan['kalori'] <= self.energy_breakdown['camilan_pagi']) & (
                    self.df_camilan['lemak'] <= max_fats_allowed) & (~self.df_camilan.index.isin(self.selected_snacks))]
        df_makan_siang_filtered = self.df_makan_siang[
            (self.df_makan_siang['kalori'] <= self.energy_breakdown['makan_siang']) & (
                        self.df_makan_siang['lemak'] <= max_fats_allowed)]
        df_makan_malam_filtered = self.df_makan_malam[
            (self.df_makan_malam['kalori'] <= self.energy_breakdown['makan_malam']) & (
                        self.df_makan_malam['lemak'] <= max_fats_allowed)]
...
def calculate_bmr_and_daily_energy(gender, weight, height, age, activity_level):
    ...
    daily_energy = bmr * activity_levels[activity_level]
